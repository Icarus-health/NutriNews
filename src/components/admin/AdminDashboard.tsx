...
export default function AdminDashboard({ drafts: initialDrafts }: Props) {
  ...
  async function toggleSourceDisabled(sourceName: string, currentlyDisabled: boolean) {
    const supabase = createClient();
    ...
    if (!error) {
      setSourceHealthList(prev =>
        prev.map(s => s.source_name === sourceName ? { ...s, disabled: !currentlyDisabled } : s)
      );
    }
  }
        ...
        <div>
          ...
          {sourceHealthLoading ? (
            ...
            <>
              {/* Summary bar */}
              {(() => {
                const disabled = sourceHealthList.filter(s => s.disabled).length;
                const active = sourceHealthList.filter(s => !s.disabled);
                const failing = active.filter(s => s.consecutive_failures > 0).length;
                const dead = active.filter(s => s.consecutive_failures >= 3).length;
                const ok = active.length - failing;
                return (
                  <div className="flex gap-2 mb-4 flex-wrap">
                    ...
                    {disabled > 0 && (
                      <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg px-3 py-1.5">
                        <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">{disabled} deaktiviert</span>
                      </div>
                    )}
                  </div>
                );
              ...
              <div className="space-y-1.5">
                {sourceHealthList.map(s => {
                  ...
                  const isDead = !isDisabled && s.consecutive_failures >= 3;
                  const isFailing = !isDisabled && s.consecutive_failures > 0 && !isDead;
                  const isOk = !isDisabled && s.consecutive_failures === 0;
                  ...
                  return (
                    <div
                      ...
                      className={clsx(
                        ...
                        isDisabled && 'bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 opacity-60',
                    ...
                    >
                      {isDisabled && <WifiOff size={14} className="shrink-0 text-slate-400" />}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className={clsx(
                            ...
                            isDisabled ? 'text-slate-400 dark:text-slate-500 line-through' :
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <button
                          onClick={() => toggleSourceDisabled(s.source_name, s.disabled)}
                          title={isDisabled ? 'Reaktivieren' : 'Deaktivieren'}
                          className={clsx(
                            'text-[10px] font-semibold px-2 py-1 rounded-lg border transition-colors shrink-0',
                            isDisabled
                              ? 'border-forest-300 dark:border-forest-700 text-forest-600 dark:text-forest-400 hover:bg-forest-50 dark:hover:bg-forest-900/20'
                              : 'border-slate-200 dark:border-slate-600 text-slate-400 hover:text-red-500 hover:border-red-300 dark:hover:border-red-700'
                          )}
                        >
                          {isDisabled ? 'An' : 'Aus'}
                        </button>
